<script setup lang="ts">
  ...
  const schema = toFieldValidator(
    z.object({
      name: z
        .string()
        .nonempty(t('alert.messages.required', { field: t('category') }))
    }),
  );
  ...
  const categoryDelete = ref<ICategory>({ name: '' } as ICategory);
  const modalOptionDelete = { template: 'del' } as ModalConfig;

  const submitForm = handleSubmit(async (values) => {
    await tryCallRequest(async () => {
      //init request
      const request = { id: formData.id, name: values.name } as ICategory;
      if (showModalUpdate.value) {
        // call request save
        await categoryStore.update(request);
      } else {
        // call request save
        await categoryStore.save(request);
      }
      // reset form
      await reset ()
    });
  ...
  async function actionDeleteProfession () {
    await tryCallRequest(async () => {
      //init request
      const request = { id: categoryDelete.value.id } as ICategory;
      // call request
      await categoryStore.remove(request);
      ...
      <div class="col-span-12 md:col-span-6">
        <form class="bg-white dark:bg-darkmode-600 rounded p-3 shadow-sm h-full" @submit='submitForm'>
          <FormLabel htmlFor="ip_address">
            <span class="font-medium text-base">{{ t('category') }}</span>
          </FormLabel>
          <div class="flex mt-3">
            <FormInput class="dark:border-slate-600" v-model='formData.name' name="name" id="name" type="text" :placeholder="t('enter', { name: t('category').toLowerCase() })" />
          </div>
      ...
      <div class="col-span-12 md:col-span-6">
        <div class="bg-white dark:bg-darkmode-600 rounded p-3 shadow-sm">
            ...
            <Table sm hover>
              <Table.Thead class="sticky top-0 bg-white dark:bg-darkmode-400 shadow-sm">
                <Table.Tr>
                  <Table.Th>
                    {{ t('category') }}
                    <span class="text-xs text-slate-400">({{ categories.length || 0 }})</span>
                  </Table.Th>
              ...
              <Table.Tbody>
                <Table.Tr v-for="category in categories" :key="category.name">
                  <Table.Td>
                  ...
                  <Table.Td class="text-right">
                    <div class="-mx-1.5">
                      <Button variant="soft-primary" size="sm" class="m-1.5" @click='formData.id = category.id; formData.name = category.name; showModalUpdate = true'>
                        <Lucide icon="Edit" class="h-4 w-4" />
                      </Button>
                      <Button variant="soft-danger" size="sm" class="m-1.5" @click='categoryDelete = category; showModalDelete = true'>
          ...
          <HeadlessUiDialogModal :open-modal="showModalDelete" @closeModal="showModalDelete = false" :modal-options="modalOptionDelete" @call-back-action="actionDeleteProfession">
            ...
            <template #modalbody>
              <p class="text-base text-danger py-2 px-3 rounded bg-light mt-1">{{ categoryDelete.name }}</p>
              <div class="mt-2 text-slate-500">
                <p>{{ t('are_you_sure_1', { action: t('delete', { name: categoryDelete.name }) }) }}</p>
