<script setup lang="ts">
  ...
  import { ModalConfig, TemplateModal } from '@/model/dto/ModalConfig';
  import { tryCallRequest } from '@/utils/my-function';
  import { MyStore } from '@/stores/my-store';

  const showRestartMySQLModal = ref(false);
  ...
  const categories = [
    {
      ...
      component: markRaw(defineAsyncComponent(() => import('@/pages/EndUser/SQLServices/DatabaseList.vue'))),
    },
    {
      title: computed(() => t('mysql_setting')),
      component: markRaw(
        defineAsyncComponent(() => import('@/pages/EndUser/SQLServices/MySQLManagerSettings.vue')),
      ),
    },
    {
      title: computed(() => t('process_manager')),
      component: markRaw(
        defineAsyncComponent(() => import('@/pages/EndUser/SQLServices/ProcessesManager.vue')),
      ),
    },
  ];
  const modalRestartMySQLOptions = {
    title: t('confirm', { name: t('restart_mysql') }),
    template: TemplateModal.CONFIRM,
  } as ModalConfig;

  async function actionRestartMySQL() {
    //call request
    await tryCallRequest(async () => {
      await databaseStore.actionRestartDatabaseService();
      //close modal
      showRestartMySQLModal.value = false;
      //alert success
...
<template>
  <div class="grid grid-cols-12 gap-6 mt-5">
      ...
      <div class="w-full">
        <Tab.Group>
          <Tab.List class="flex space-x-1 rounded-md bg-secondary dark:bg-darkmode-900 p-1">
            <Tab v-for="(cate, index) in categories" :key="index" v-slot="{ selected }" as="template">
              <button
                :class="[
    ...
    <div class="col-span-1 md:col-span-4 lg:col-span-3 relative">
      <Tippy
        ...
        <Lucide v-if="!infomationMobileToggle" icon="Info" class="w-6 h-6" />
        <Lucide v-else icon="X" class="w-6 h-6" />
      </Tippy>
      <div
        ...
        <div class="text-right">
          <Tippy :as="Button" content="Restart MySQL" variant="warning" @click="showRestartMySQLModal = true">
  ...
  <HeadlessUiDialogModal
    :open-modal="showRestartMySQLModal"
    :modal-options="modalRestartMySQLOptions"
    @close-modal="showRestartMySQLModal = false"
    @call-back-action="actionRestartMySQL">
    <template #modalbody>
      <p>
        {{ t('are_you_sure', { action: t('restart_mysql') }) }}
      </p>
    </template>
  </HeadlessUiDialogModal>
</template>
